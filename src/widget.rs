...
use bon::builder;
use container::Mode;
pub use container::{Container, Contents};
...
pub trait Widget: Renderable {
    ...
    fn mode<S>(self) -> Mode<S>
    where
        Self: Sized + 'static,
    {
        Mode::Stateless(Box::new(self))
    }
}
...
pub trait StatefulWidget: Renderable {
    type State;

    fn dispatch(&mut self, _event: &Event, _state: &mut Self::State) -> Result<Broadcast> {
        Ok(Broadcast::Ignored)
    }

    fn draw(&mut self, frame: &mut Frame, area: Rect, state: &mut Self::State) -> Result<()>;

    fn mode(self) -> Mode<Self::State>
    where
        Self: Sized + 'static,
    {
        Mode::Stateful(Box::new(self))
    }
}

impl<S> std::fmt::Debug for Box<dyn StatefulWidget<State = S>> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct(
            format!(
                "Box<dyn StatefulWidget<{}<State = {}>>>",
                self._name(),
                std::any::type_name::<Option<S>>(),
            )
            .as_str(),
        )
        .finish()
